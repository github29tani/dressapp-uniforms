...
import { MOCK_SCHOOLS, MOCK_PRODUCTS, MOCK_CATEGORIES } from "@/lib/mock-data"
...
export default function HomePage() {
  const featuredSchools = MOCK_SCHOOLS.slice(0, 6)
        ...
        <div className="relative container mx-auto px-4 py-16 md:py-28">
          <div className="max-w-xl">
            <div className="inline-flex items-center gap-2 bg-white/15 border border-white/25 text-white text-xs font-medium px-3 py-1.5 rounded-full mb-6">
              ...
              100+ Schools &nbsp;·&nbsp; 500+ Products &nbsp;·&nbsp; Trusted by parents
            </div>
      ...
      <section className="container mx-auto px-4 py-16">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-500 mt-2 text-sm max-w-sm mx-auto">
            No more searching multiple shops. Get everything your child needs in one order.
          </p>
        </div>
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto">
          ...
          {[
            { step: "1", title: "Select Your School", desc: "Search from 100+ schools and pick yours.", color: "bg-blue-700", ring: "ring-blue-100" },
            ...
            { step: "3", title: "Pick Sizes & Order", desc: "Add the complete kit to cart and checkout in minutes.", color: "bg-emerald-600", ring: "ring-emerald-100" },
          ].map(({ step, title, desc, color, ring }) => (
            <div key={step} className="relative z-10 flex flex-col items-center text-center gap-4">
              <div className={`w-16 h-16 rounded-2xl ${color} ring-4 ${ring} text-white text-2xl font-extrabold flex items-center justify-center shadow-lg`}>
                {step}
              </div>
              <div>
        ...
        <div className="text-center mt-10">
          <LinkButton href="/schools" size="lg" className="bg-blue-700 hover:bg-blue-800 font-semibold shadow-md shadow-blue-200">
            Get Started — Select Your School
            <ArrowRight className="h-4 w-4 ml-2" />
          </LinkButton>
        </div>
      </section>
            ...
            {MOCK_CATEGORIES.map((cat) => (
              <Link key={cat.id} href={`/products?category=${cat.slug}`}>
                <div className="group flex flex-col items-center gap-2.5 p-4 rounded-2xl bg-white border border-gray-100 hover:border-blue-300 hover:shadow-md transition-all cursor-pointer text-center">
                  ...
                  <p className="text-xs font-semibold text-gray-700 group-hover:text-blue-700 transition-colors leading-tight">
                    {cat.name}
                  </p>
                </div>
              </Link>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-700 via-blue-800 to-indigo-800 p-10 md:p-14 text-white text-center">
          ...
          <div className="relative">
            ...
            <p className="text-blue-100 mb-8 max-w-md mx-auto text-sm leading-relaxed">
              Partner with us for bulk uniform orders, custom branding, embroidery, and school merchandise for your students.
            </p>
            <div className="flex flex-wrap gap-3 justify-center">
              <LinkButton href="/bulk-orders" size="lg" className="bg-white text-blue-700 hover:bg-blue-50 font-bold shadow-lg">
                Request a Quote
              </LinkButton>
              <LinkButton href="/school-partnerships" size="lg" variant="outline" className="border-white/40 text-white hover:bg-white/10">
                School Partnerships
              </LinkButton>
            </div>
          </div>
